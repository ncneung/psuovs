...
Public Class TEST
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim classBallot = New clsRegisted
        Dim classCand = New clsCandidate
        Dim classCandDetail = classCand.getCandidate(1)

        'Repeater1.DataSource = classBallot.getBallotsForVoter("5735512073")
        'Repeater1.DataBind().

        'RadioButtonList1.DataTextField = "BallotsID"
        'RadioButtonList1.DataValueField = "BallotsID"

        'RadioButtonList1.DataSource = classBallot.getBallotsForVoter("5735512073")
        'RadioButtonList1.DataBind()
        RadioButtonList1.TextAlign = TextAlign.Left
        RadioButtonList1.CellSpacing = 100
        RadioButtonList1.CellPadding = -1
        For X As Integer = 0 To 2
            'RadioButtonList1.Items.Add(New ListItem(classCandDetail(X).CandidateNumber & "  " & classCandDetail(X).NameTH & "  " & classCandDetail(X).NameEN, classCandDetail(X).CandidateID))
            RadioButtonList1.Items.Add(New ListItem("", classCandDetail(X).CandidateID))
        Next
